...
struct ContextBundle {
    ...
    /// Self-page slug, env-overridable — mirrors the MCP's GLIA_SELF_SLUG so the
    /// OSS multi-user build front-loads the actual user's self-page, not "david".
    nonisolated static var selfSlug: String {
        (ProcessInfo.processInfo.environment["GLIA_SELF_SLUG"] ?? "people-david").lowercased()
    }

    nonisolated static func identityRank(_ node: BrainNode) -> Int {
        let s = node.slug.lowercased()
        // self-page in either slug form (people-david ↔ people/david), config-driven
        if s == selfSlug || s == selfSlug.replacingOccurrences(of: "-", with: "/") { return 0 }
    ...
    /// `nonisolated` and reads only Sendable inputs, so callers can run it off
    /// the main actor (it does synchronous disk I/O over potentially many files).
    /// `mirrors` maps a source id → its on-disk mirror base URL; resolve it on
    /// the main actor (via `BrainLocation.sourceMirror`) before calling.
    nonisolated static func build(nodes: [BrainNode], header: String,
                                  mirrors: [String: URL]) -> ContextBundle {
        var parts: [String] = []
        parts.append("# \(header)")
        parts.append("<!-- Generated by Glia — a map of the mind for context injection. -->\n")
        var count = 0
        var seen = Set<String>()   // a slug present in two enabled sources must not
                                   // be injected twice (wastes budget, double-weights,
                                   // inflates pageCount). nodes are identityRank-sorted,
                                   // so the first (highest-priority) occurrence wins.
        for node in nodes {
